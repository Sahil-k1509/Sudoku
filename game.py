...
class Grid:
    ...
    def __init__(self, rows, cols, width, height):
        ...
        self.cols = cols
        self.cubes = [
            [ Cube(self.board[i][j], i, j, width, height) for j in range(cols) ] for i in range(rows)
        ]
        self.width = width
        self.height = height
        self.model = None
    ...
    def is_finished(self):
        for i in range(self.rows):
            for j in range(self.cols):
                if self.cubes[i][j].value == 0: return False
...
def main():
    ...
    while run:  
        play_time = round(time.perf_counter() - start)
